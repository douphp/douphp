...
class ArticleFormRequest extends FormRequest
{
    ...
    /**
     * 表单规则（请求层）：
     * - 定义输入校验规则（required/integer/date 等）。
     * - 同时作为 FormRequest::validated() 的字段白名单键集合。
     *
     * 与 Article::$fillable 的分工：
    ...
    public function rules()
    {
        $rules = array(
            ...
            // 仅用于处理内容中远程图片本地化的行为开关，不是 article 表字段
            'content_remote_image_local' => 'boolean',
            // 草稿附件认领令牌：仅 store 场景必填，由 create() action 通过
            // attachment()->newDraftToken() 生成并随表单透传；update 场景无意义
            'draft_token' => '',
        );
        ...
        if (!empty(Config::get('features.slug', false))) {
            $rules['slug'] = $this->scene === 'update'
                ? 'required|slug|unique:article,slug,id'
                : 'required|slug|unique:article,slug';
        }
