...
/**
 * 单（无分类）模块导出 trait：为 Sitemap / Llms 等动态聚合提供列表行（listForExport）。
 *
 * 输出契约 title / name / description / created_at / url：多语言经 langBox 覆写、
 * description 缺省取正文 320 字摘要、url 走 route($module . '.show')。
 * status 过滤由 scopePublished 守护；不含分类过滤。
 */
trait HasExportableSingle
{
    /**
     * 导出用内容列表。
     *
     ...
     */
    public static function listForExport($limit = 0)
    {
    ...
    protected static function toExportRow(array $raw)
    {
        ...
        return array(
            ...
            'created_at' => $addTime ? date('Y-m-d', $addTime) : '',
