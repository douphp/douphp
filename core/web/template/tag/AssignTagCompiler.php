...
/**
 * {@see \Dou\Core\Web\Template\Ast\NodeType::ASSIGN} 标签编译器：
 * {assign var=... value=...} → $ctx->set(...)。
 */
class AssignTagCompiler implements TagCompilerInterface
{
    /**
     * @inheritDoc
     */
    public function compile(Node $node, TagCompileContext $ctx)
    {
        ...
        $ctx->emit('<?php $ctx->set(' . $attrs['var'] . ', ' . $attrs['value'] . '); ?>');
    }
}
