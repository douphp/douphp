...
class PHPExcel_Writer_PDF implements PHPExcel_Writer_IWriter
{
    ...
    {
        $pdfraryName = PHPExcel_Settings::getPdfRendererName();
        if (is_null($pdfraryName)) {
            throw new PHPExcel_Writer_Exception("PDF Rendering library has not been defined.");
        }

        $pdfraryPath = PHPExcel_Settings::getPdfRendererPath();
        if (is_null($pdfraryName)) {
            throw new PHPExcel_Writer_Exception("PDF Rendering library path has not been defined.");
        }
        $includePath = str_replace('\\', '/', get_include_path());
        $rendererPath = str_replace('\\', '/', $pdfraryPath);
        if (strpos($rendererPath, $includePath) === false) {
            set_include_path(get_include_path() . PATH_SEPARATOR . $pdfraryPath);
        }

        $rendererName = 'PHPExcel_Writer_PDF_' . $pdfraryName;
