...
//        }
        ...
        if ($zip->open($pFilename) === true) {
            ...
            if ($stat && ($stat['size'] <= 255)) {
                ...
                $xml = simplexml_load_string($this->securityScan($zip->getFromName('META-INF/manifest.xml')), 'SimpleXMLElement', PHPExcel_Settings::getXmlLoaderOptions());
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $xml = new XMLReader();
        $res = $xml->xml($this->securityScanFile('zip://'.realpath($pFilename).'#content.xml'), null, PHPExcel_Settings::getXmlLoaderOptions());
...
//        echo '<h1>Meta Information</h1>';
        $xml = simplexml_load_string($this->securityScan($zip->getFromName("meta.xml")), 'SimpleXMLElement', PHPExcel_Settings::getXmlLoaderOptions());
...
//        echo '<h1>Workbook Content</h1>';
        $xml = simplexml_load_string($this->securityScan($zip->getFromName("content.xml")), 'SimpleXMLElement', PHPExcel_Settings::getXmlLoaderOptions());
        $namespacesContent = $xml->getNamespaces(true);
//        echo '<pre>';
