...
/**
 * 一对一关系：关联模型持外键（foreignKey）指向父模型的键（localKey）。
 */
class HasOne extends Relation
{
    ...
    public function match(array $models, $results, $name)
    {
        ...
        foreach ($results->all() as $result) {
            $key = (string) $result->getRawAttribute($this->foreignKey);
        ...
        foreach ($models as $model) {
            ...
            $model->setRelation($name, isset($dictionary[$local]) ? $dictionary[$local] : null);
        }
    }
