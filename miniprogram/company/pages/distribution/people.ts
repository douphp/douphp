...
Page({
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      .get(route('distribution.user.people'), {
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.people_list
          const dataNew = data.people_list
          if (dataNew) {
          ...
          dataBox = data.people_list
        }

        that.setData({ people_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
