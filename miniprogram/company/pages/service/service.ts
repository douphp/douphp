...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('service') })
    wx.setStorageSync('shareTitle', pageTitle('service'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.service_list
          const dataNew = data.service_list
          if (dataNew) {
          ...
          dataBox = data.service_list
        }

        that.setData({ service_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
