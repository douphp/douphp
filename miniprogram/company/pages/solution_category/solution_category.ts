// pages/solution_category/solution_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('solution') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.solution_list
          const dataNew = data.solution_list
          if (dataNew) {
          ...
          dataBox = data.solution_list
        }

        that.setData({
          solution_list: dataBox,
