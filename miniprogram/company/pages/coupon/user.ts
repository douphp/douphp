...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('coupon_user') })
    wx.setStorageSync('shareTitle', pageTitle('coupon_user'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.coupon_list
          const dataNew = data.coupon_list
          if (dataNew) {
          ...
          dataBox = data.coupon_list || []
        }

        that.setData({ coupon_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
