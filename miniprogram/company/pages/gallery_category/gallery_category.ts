// pages/gallery_category/gallery_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('gallery') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.gallery_list
          const dataNew = data.gallery_list
          if (dataNew) {
          ...
          dataBox = data.gallery_list
        }

        that.setData({
          gallery_list: dataBox,
