...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('form') })
    wx.setStorageSync('shareTitle', pageTitle('form'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.form_list
          const dataNew = data.form_list
          if (dataNew) {
          ...
          dataBox = data.form_list
        }

        that.setData({ form_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
