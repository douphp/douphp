...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('chat_history') })
    wx.setStorageSync('shareTitle', pageTitle('chat_history'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      .get(route('chat.user.history'), {
        page: page ? page : that.data.page,
      })
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.session_list
          const dataNew = data.session_list
          ...
          dataBox = data.session_list
        }

        that.setData({ session_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
