// pages/chat/chat.ts AI助手默认应用对话页（流式；无可用默认应用时回落首页，与 WEB /chat 一致）
...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('chat') })
    wx.setStorageSync('shareTitle', pageTitle('chat'))
  ...
  loadApp() {
    ...
    http
      ...
      .then(function (data) {
        ...
        that.setData({
          title: data.chat ? data.chat.name : pageTitle('chat'),
          chat: data.chat,
        ...
        wx.setStorageSync('shareTitle', data.chat ? data.chat.name : pageTitle('chat'))
