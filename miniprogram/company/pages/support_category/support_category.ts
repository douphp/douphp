// pages/support_category/support_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('support') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.support_list
          const dataNew = data.support_list
          if (dataNew) {
          ...
          dataBox = data.support_list
        }

        that.setData({
          support_list: dataBox,
