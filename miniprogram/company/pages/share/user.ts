...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('share_user') })
    wx.setStorageSync('shareTitle', pageTitle('share_user'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      .get(route('share.user'), {
        page: page ? page : that.data.page,
      })
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.share_list
          const dataNew = data.share_list
          if (dataNew) {
  ...
  applyExist() {
    douMsg(this.data.lang.share_apply_exist)
  },
