// pages/article_category/article_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('article') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.article_list
          const dataNew = data.article_list
          if (dataNew) {
