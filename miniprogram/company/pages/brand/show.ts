...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('brand_show') })
    ...
    http
      .get(route('brand.show', { id: options.id }))
      .then(function (data) {
        that.setData({ brand: data.brand })
        wx.setStorageSync('shareTitle', data.brand ? data.brand.name : '')
      })
      .catch(function (err) {
