...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('point_user') })
    wx.setStorageSync('shareTitle', pageTitle('point_user'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        that.setData({ log_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
