// pages/professional_category/professional_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('professional') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.professional_list
          const dataNew = data.professional_list
          if (dataNew) {
          ...
          dataBox = data.professional_list
        }

        that.setData({
          professional_list: dataBox,
