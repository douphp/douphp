// pages/cases_category/cases_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('cases') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.cases_list
          const dataNew = data.cases_list
          if (dataNew) {
          ...
          dataBox = data.cases_list
        }

        that.setData({
          cases_list: dataBox,
          cases_category: data.cases_category,
