...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('withdraw_user') })
    wx.setStorageSync('shareTitle', pageTitle('withdraw_user'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      .get(route('withdraw.user'), {
        page: page ? page : that.data.page,
      })
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.withdraw_list
          const dataNew = data.withdraw_list
          if (dataNew) {
          ...
          dataBox = data.withdraw_list
        }

        that.setData({
          withdraw_list: dataBox,
          money_total: data.money_total,
          apply_exist: data.apply_exist,
          loadpage: false,
        })
      })
      .catch(function (err) {
  ...
  applyExist() {
    douMsg(this.data.lang.withdraw_apply_exist)
  },
