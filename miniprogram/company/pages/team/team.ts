...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('team') })
    wx.setStorageSync('shareTitle', pageTitle('team'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.team_list
          const dataNew = data.team_list
          if (dataNew) {
