// pages/certificate/certificate.ts
...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('certificate') })
    wx.setStorageSync('shareTitle', pageTitle('certificate'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let imageBox
        if (concat) {
          dataBox = that.data.certificate_list
          const dataNew = data.certificate_list
          if (dataNew) {
          ...
          imageBox = that.data.certificate_list
          const imageNew = data.certificate_list
          if (imageNew) {
            imageBox = imageBox.concat(imageNew)
          }
        } else {
          dataBox = data.certificate_list
          imageBox = data.image_list
        }

        that.setData({
          certificate_list: dataBox,
          image_list: imageBox,
          loadpage: false,
        })
      })
