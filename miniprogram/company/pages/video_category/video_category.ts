// pages/video_category/video_category.ts
...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('video') })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        let dataBox
        if (concat) {
          dataBox = that.data.video_list
          const dataNew = data.video_list
          if (dataNew) {
