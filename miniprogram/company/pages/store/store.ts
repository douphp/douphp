...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('store') })
    wx.setStorageSync('shareTitle', pageTitle('store'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.store_list
          const dataNew = data.store_list
          if (dataNew) {
