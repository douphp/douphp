// pages/health/user.ts — 会员中心健康档案列表（route=health/user）
...
Page({
  ...
  onLoad() {
    this.setData({ title: pageTitle('health_user') || pageTitle('health') || '健康档案' })
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        ...
        that.setData({ health_list: list, loadpage: false })
      })
      .catch(function (err) {
