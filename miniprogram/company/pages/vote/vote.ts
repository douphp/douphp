...
Page({
  ...
  onLoad() {
    ...
    that.setData({ title: pageTitle('vote') })
    wx.setStorageSync('shareTitle', pageTitle('vote'))
  ...
  loadData(concat = false, page: number | string = '') {
    ...
    http
      ...
      .then(function (data) {
        let dataBox
        if (concat) {
          dataBox = that.data.vote_list
          const dataNew = data.vote_list
          if (dataNew) {
          ...
          dataBox = data.vote_list
        }

        that.setData({ vote_list: dataBox, loadpage: false })
      })
      .catch(function (err) {
