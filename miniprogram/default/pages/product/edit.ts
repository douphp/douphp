...
Page({
  ...
  onLoad(options) {
    ...
    that.setData({ title: pageTitle('product_edit') })
    wx.setStorageSync('shareTitle', pageTitle('product_edit'))
  ...
  productAction(e: WechatMiniprogram.CustomEvent) {
    ...
    authStore.ensureLogin().then(function (ok) {
      ...
      checkWorkPermission('product')
      http
        .put(route('product.work.update', { id: that.data.id || 0 }), {
          id: that.data.id || 0,
